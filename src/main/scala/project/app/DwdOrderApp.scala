...
import project.model.{OrderDetail, OrderInfo, OrderWide}
import redis.clients.jedis.Jedis
import tools.{EsTools, KafkaTools, OffsetsTools, RedisTools}
...
object DwdOrderApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val orderInfoOffsets: Map[TopicPartition, Long] =
      OffsetsTools.readOffset(orderInfoTopicName, orderInfoGroup)
    // order_detail
    ...
    val orderDetailOffsets: Map[TopicPartition, Long] =
      OffsetsTools.readOffset(orderDetailTopicName, orderDetailGroup)
    ...
    if (orderInfoOffsets != null && orderInfoOffsets.nonEmpty) {
      orderInfoKafkaDStream = KafkaTools.getKafkaDStream(ssc, orderInfoTopicName, orderInfoGroup, orderInfoOffsets)
    } else {
      orderInfoKafkaDStream = KafkaTools.getKafkaDStream(ssc, orderInfoTopicName, orderInfoGroup)
    }
    ...
    if (orderDetailOffsets != null && orderDetailOffsets.nonEmpty) {
      orderDetailKafkaDStream = KafkaTools.getKafkaDStream(ssc, orderDetailTopicName, orderDetailGroup, orderDetailOffsets)
    } else {
      orderDetailKafkaDStream = KafkaTools.getKafkaDStream(ssc, orderDetailTopicName, orderDetailGroup)
    }
    ...
    val orderInfoDimDStream: DStream[OrderInfo] = orderInfoDStream.mapPartitions(
      orderInfoIter => {
        ...
        val jedis: Jedis = RedisTools.getJedisFromPool()
        ...
        rdd.foreachPartition(
          orderWideIter => {
            ...
            if (orderWides.size > 0) {
              ...
              // 写入到ES
              EsTools.bulkSave(indexName, orderWides)
            }
          }
        )
        // 提交offsets
        OffsetsTools.saveOffset(orderInfoTopicName, orderInfoGroup, orderInfoOffsetRanges)
        OffsetsTools.saveOffset(orderDetailTopicName, orderDetailGroup, orderDetailOffsetRanges)
      }
    )
