...
import project.model.{DauInfo, PageLog}
...
object DwdDauApp {
    ...
    val dauInfoDStream: DStream[DauInfo] = redisFilterDStream.mapPartitions(
      pageLogIter => {
        ...
        for (pageLog <- pageLogIter) {
          ...
          // 好办法: 通过对象拷贝来完成.
          BeanTools.copyProperties(pageLog, dauInfo)
        ...
        rdd.foreachPartition(
          dauInfoIter => {
            ...
            if (docs.size > 0) {
              ...
              // 写入到ES中
              EsTools.bulkSave(indexName, docs)
            }
          }
        )
        // 提交offset
  ...
  def revertState(): Unit = {
    ...
    val mids: List[String] = EsTools.searchField(indexName, fieldName)
